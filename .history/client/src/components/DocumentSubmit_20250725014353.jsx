...
export default function CartPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const orderDetails = {
        documentIds,
        deliveryAddress: "123 Main Street, Bandra East, Mumbai",
  ...
  // --- Empty Cart State ---
  if (!loading && !error && documents.length === 0 && !orderPlaced) {
    return (
      <div className="min-h-screen bg-gray-950 p-6 flex flex-col justify-center items-center text-gray-100 font-sans">
        <div className="text-center p-12 bg-gray-800 rounded-xl shadow-2xl max-w-lg w-full mx-auto border border-blue-700">
          <Info className="w-20 h-20 mx-auto text-blue-500 mb-6 drop-shadow-lg" />
          <h2 className="text-3xl font-serif font-bold mb-4 text-white">Your Cart Is Empty</h2>
          <p className="text-lg mb-6 text-gray-400">
            Looks like you haven't added any documents yet.
          </p>
          <button
            onClick={() => navigate('/print-section')} // Changed to navigate
  ...
  return (
    <div className="min-h-screen bg-gray-950 p-6 text-gray-100 font-sans relative">
      ...
      <div className="max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
        <div className="lg:col-span-2 space-y-6">
          <h1 className="text-4xl font-serif font-bold mb-6 flex items-center gap-3 text-white">
          ...
          {error && (
            <div className="bg-red-900 text-red-200 border border-red-700 rounded-lg p-4 flex items-center gap-3 shadow-md">
          ...
          {documents.length > 0 && (
            <div className="grid grid-cols-1 gap-5">
              {documents.map(doc => (
                <div key={doc._id} className="card bg-gray-800 rounded-xl shadow-lg border border-gray-700 hover:border-blue-500 transition-all duration-200">
                  <div className="card-body flex flex-col md:flex-row items-center gap-6 p-6">
                    <div className="w-32 h-32 flex-shrink-0 rounded-lg overflow-hidden border border-gray-600 shadow-inner">
                      ...
                      doc.fileUrl?.endsWith('.jpeg') ? (
                        ...
                        <div className="bg-blue-700 text-white w-full h-full flex items-center justify-center text-5xl font-bold">
                    ...
                    <div className="flex-grow text-center md:text-left">
                      <h2 className="text-2xl font-semibold text-white mb-2">{doc.originalName}</h2>
                      <div className="flex flex-wrap justify-center md:justify-start gap-3 text-sm mt-3">
        ...
        {/* --- Order Summary --- */}
        <div className="bg-gray-800 shadow-2xl rounded-xl p-8 flex flex-col gap-5 self-start sticky lg:top-6 border border-blue-700">
          <h2 className="text-2xl font-serif font-bold text-white border-b border-gray-700 pb-4 mb-2">Order Summary</h2>
          ...
          <div className="flex justify-between text-2xl font-bold text-blue-400 border-t border-gray-700 pt-4 mt-4">
          ...
          <div className="mt-6">
            ...
            <select
              className="select select-bordered w-full mb-6 bg-gray-700 border-gray-600 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all duration-200 rounded-lg py-4 text-lg"
              value={paymentType}
