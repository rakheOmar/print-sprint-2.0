...
// New, more robust function to calculate marker movement for smooth animation
// It calculates the distance to move per frame based on remaining distance and ETA
const moveMarkerSmoothly = (currentPos, targetPos, distanceToTargetMeters, remainingEtaMinutes, intervalMs) => {
  ...
  // Calculate the ratio of the distance to move in this frame to the total remaining distance
  const fractionOfRemainingPath = actualDistanceToMove / distanceToTargetMeters;

  // Linearly interpolate the new latitude and longitude
  const newLat = currentLat + (targetLat - currentLat) * fractionOfRemainingPath;
  const newLon = currentLon + (targetLon - currentLon) * fractionOfRemainingPath;
      ...
      setOrderExtras((prev) => {
        ...
        for (const id in updated) {
          if (!updated[id].reached) {
            ...
            // Determine if the marker has reached the target (e.g., within 10 meters)
            ...
            // Adjust ETA decrement for display (can still accelerate for visual effect)
            let etaDecrement = 0.05; // Base decrement per 100ms interval
            ...
            updated[id] = {
              ...updated[id],
              marker: newPos, // Use the position calculated by moveMarkerSmoothly
              reached: reached,
              eta: reached ? 0 : Math.max(0, currentEta - etaDecrement),
            };

            if (!reached) {
              anyOrderStillTracking = true; // Keep tracking if not yet reached
            }
          }
        }
