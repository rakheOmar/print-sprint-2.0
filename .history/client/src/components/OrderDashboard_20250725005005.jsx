...
const moveMarkerSmoothly = (currentPos, targetPos, distanceToTargetMeters, remainingEtaMinutes, intervalMs) => {
  ...
  // Convert remaining ETA to milliseconds. Cap at a reasonable minimum (e.g., 1 second)
  // to prevent division by zero or extremely large frame counts for ETA near 0.
  const totalTimeRemainingMs = Math.max(1000, remainingEtaMinutes * 60 * 1000); // Minimum 1 second for ETA
  ...
  // Calculate initial bearing (direction) from current to target
  // This is the bearing for the *entire* path from current to target
...
const UserDashboard = () => {
  ...
  // --- DEBUGGING HELPERS (now using useRef for stability) ---
  const firstOrderIdToLogRef = useRef(null); // Use useRef for order ID to log
  const logCounterRef = useRef(0); // Use useRef for log counter
  const LOG_INTERVAL = 10; // Log every 10 frames
  // -------------------------
    ...
    try {
      ...
      if (response.ok) {
        ...
        result.data.forEach((order) => {
          // Reduced initial random distance and ETA for better visibility
          const randomDistanceKm = 0.1 + Math.random() * 0.5; // From 0.1km (100m) to 0.6km (600m)
          ...
          extras[order._id] = {
            ...
            eta: Math.floor(Math.random() * 5) + 1, // Initial ETA between 1 and 5 minutes
            reached: false,
          };
        });
  ...
  useEffect(() => {
    const animationInterval = setInterval(() => {
      setOrderExtras((prev) => {
        ...
        // Update logCounterRef directly, does not cause re-render
        ...
        for (const id in updated) {
          if (!updated[id].reached) {
            ...
            // --- DEBUG LOGS (only for one order, every N frames) ---
            if (!firstOrderIdToLogRef.current) { // Capture the ID of the first order in the loop
              firstOrderIdToLogRef.current = id;
            }

            if (id === firstOrderIdToLogRef.current && logCounterRef.current % LOG_INTERVAL === 0) {
              console.groupCollapsed(`DEBUG: Order ${id.slice(-6).toUpperCase()} - Frame ${logCounterRef.current}`);
              console.log(`  Current Marker Pos: [${currentMarkerPos[0].toFixed(6)}, ${currentMarkerPos[1].toFixed(6)}]`);
              console.log(`  Target Pos: [${coords[0].toFixed(6)}, ${coords[1].toFixed(6)}]`);
              console.log(`  Current ETA (min): ${currentEta.toFixed(3)}`);
              console.log(`  Distance to Target (m): ${distanceToTargetMeters.toFixed(3)}`);
              console.log(`  Reached Threshold (10m): ${reached}`);

              // Parameters for moveMarkerSmoothly to verify internally
              const totalTimeRemainingMs = Math.max(1000, currentEta * 60 * 1000); // Minimum 1 second for ETA
              const expectedFramesLeft = Math.max(1, Math.ceil(totalTimeRemainingMs / 100)); // 100 is intervalMs
              const distanceToCoverThisFrame = distanceToTargetMeters / expectedFramesLeft;
              const actualDistanceToMove = Math.min(distanceToCoverThisFrame, distanceToTargetMeters);

              console.log(`  Animation Interval (ms): 100`);
              console.log(`  Total Time Remaining (ms for ETA): ${totalTimeRemainingMs.toFixed(0)}`);
              console.log(`  Expected Frames Left: ${expectedFramesLeft}`);
              console.log(`  Distance to Cover This Frame (m): ${distanceToCoverThisFrame.toFixed(6)}`);
              console.log(`  Actual Distance To Move (m): ${actualDistanceToMove.toFixed(6)}`);

              // Verify the output of moveMarkerSmoothly
              console.log(`  Calculated New Pos (from fn): [${newPos[0].toFixed(6)}, ${newPos[1].toFixed(6)}]`);
              console.log(`  Change in Lat: ${(newPos[0] - currentMarkerPos[0]).toFixed(7)}`);
              console.log(`  Change in Lon: ${(newPos[1] - currentMarkerPos[1]).toFixed(7)}`);
              console.groupEnd();
            }
            // --- END DEBUG LOGS ---
          }
  ...
  }, [coords]); // Only 'coords' as dependency, ensuring interval stability
