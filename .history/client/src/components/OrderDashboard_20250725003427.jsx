...
const UserDashboard = () => {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-950"> {/* Dark background for loading */}
        <span className="loading loading-spinner loading-lg text-gray-400"></span> {/* Lighter spinner */}
        <p className="ml-3 text-lg text-gray-400">Loading orders...</p> {/* Lighter text */}
      </div>
    );
  }

  return (
    <div className="p-6 max-w-6xl mx-auto bg-gray-950 min-h-screen text-gray-100"> {/* Deep black background, light text */}
      <h1 className="text-4xl font-bold text-center mb-8 text-white">📦 My Orders</h1> {/* White title */}

      {orders.length === 0 ? (
        <p className="text-center text-gray-400 text-lg">No orders found.</p> // Lighter gray for no orders
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {orders.map((order) => {
            ...
            return (
              <div
                key={order._id}
                className="card bg-gray-800 text-gray-100 shadow-xl border border-gray-700 rounded-lg overflow-hidden" // Dark card, lighter border
              >
                <div className="card-body p-6">
                  <h2 className="card-title text-xl font-bold text-blue-400 mb-2 flex items-center gap-2"> {/* Softer blue for title */}
                    <PackageCheck className="w-6 h-6 text-blue-300" /> {/* Lighter blue icon */}
                    Order #{order._id.slice(-6).toUpperCase()}
                  </h2>

                  <div
                    className={`badge ${isDelivered ? 'bg-green-600 text-white' : 'bg-blue-600 text-white'} mt-2 px-3 py-1 rounded-full text-xs font-semibold uppercase`} // Stronger green/blue for badges
                  >
                    {isDelivered ? 'delivered' : order.status}
                  </div>

                  <div className="mt-5 space-y-3 text-sm text-gray-300"> {/* Lighter gray for details text */}
                    <p className="flex items-center gap-3">
                      <MapPin className="w-5 h-5 text-purple-400" /> {/* Softer purple icon */}
                      <span className="font-semibold">Address:</span>{' '}
                      {location || order.deliveryAddress}
                    </p>
                    <p className="flex items-center gap-3">
                      <IndianRupee className="w-5 h-5 text-green-500" /> {/* Softer green icon */}
                      <span className="font-semibold">Total:</span> ₹
                      {order.totalAmount}
                    </p>
                    <p className="flex items-center gap-3">
                      <CreditCard className="w-5 h-5 text-indigo-400" /> {/* Softer indigo icon */}
                      <span className="font-semibold">Payment:</span>{' '}
                      {order.paymentType} {order.isPaid && '(Paid)'}
                    </p>
                    <p className="flex items-center gap-3">
                      <CalendarDays className="w-5 h-5 text-orange-400" /> {/* Softer orange icon */}
                      <span className="font-semibold">Date:</span>{' '}
                      {new Date(order.createdAt).toLocaleString()}
                    </p>
                    {extras.deliveryPerson && (
                      <p className="flex items-center gap-3">
                        <UserIcon className="w-5 h-5 text-teal-400" /> {/* Softer teal icon */}
                    ...
                    {!isDelivered ? (
                      <p className="flex items-center gap-3 text-red-400 font-bold"> {/* Softer red for ETA */}
                        ...
                        <p className="flex items-center gap-3 text-green-500 font-bold"> {/* Still green for delivered */}
                  ...
                  <div className="card-actions mt-6 justify-end">
                    <button
                      className="btn btn-sm flex items-center gap-2 px-4 py-2 rounded-md transition duration-200 bg-gray-700 text-white border-gray-600 hover:bg-gray-600 hover:border-gray-500" // Darker button
                      onClick={() =>
                        ...
                        >
                          ...
                          {!isDelivered && (
                            <>
                              ...
                              <Polyline positions={[coords, extras.marker]} color="#42a5f5" weight={3} /> {/* A slightly brighter blue for visibility */}
                            </>
                          )}
                        </MapContainer>
                      </div>
